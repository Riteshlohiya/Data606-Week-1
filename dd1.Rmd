...
#Set my working directory as C:\Users\Arnav\Documents\R\win-library\3.4\DATA606\labs\Lab0\more #
...
Answer: There is dip in # of girls baptized from 1640 till 1660. But after that # of girls baptized increased.  
...
```{r plot-year-vs-propboys, eval=TRUE}
...
```{r dim-present, eval=TRUE}
dim(present)
```


```{r names-present, eval=TRUE}
names(present)
```
...
```{r summ-arbuthnot, eval=TRUE}
summary(arbuthnot)
```

```{r summ-present, eval=TRUE}
summary(present)
```
...
```{r plot-boys-vs-year, eval=TRUE}
plot(present$year, (present$boys/(present$boys + present$girls)),type = 'l')
```  

Answer: Proportion of boys is greater than proportion of girls.

-   In what year did we see the most total number of births in the U.S.?

```{r Max-year, eval=TRUE}
